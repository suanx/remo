...
use remo::ext_permission::{PermissionAction, PermissionPlugin, PermissionPolicyKey};
...
impl AgentResolver for PermissionResolver {
    fn resolve(&self, _agent_id: &str) -> Result<ResolvedAgent, RuntimeError> {
        ...
        let plugins: Vec<Arc<dyn remo::plugins::Plugin>> = vec![Arc::new(PermissionPlugin)];
...
async fn main() {
    ...
    {
        let mut patch = remo::state::MutationBatch::new();
