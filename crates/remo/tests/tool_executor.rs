...
use remo::agent::state::{RunLifecycle, SetInferenceOverride, ToolCallStates};
...
use remo::contract::suspension::{ToolCallOutcome, ToolCallStatus};
use remo::contract::tool::{
    Tool, ToolCallContext, ToolDescriptor, ToolError, ToolOutput, ToolResult,
};
use remo::execution::ParallelToolExecutor;
use remo::loop_runner::{AgentLoopParams, build_agent_env, run_agent_loop};
use remo::registry::AgentSpec;
use remo::*;
use remo::{AgentResolver, ExecutionEnv, ResolvedAgent, RuntimeError};
...
async fn terminate_via_state_in_after_inference_hook() {
    use remo::agent::state::{RunLifecycle, RunLifecycleUpdate};
...
async fn before_inference_hook_override_reaches_request() {
    use remo::contract::inference::InferenceOverride;
...
async fn context_message_injected_into_request() {
    ...
    // Base system prompt
    assert_eq!(msgs[0].role, remo::contract::message::Role::System);
    assert_eq!(msgs[0].text(), "sys");

    // Injected system context
    assert_eq!(msgs[1].role, remo::contract::message::Role::System);
    assert_eq!(msgs[1].text(), "You are a helpful assistant");

    // User message
    assert_eq!(msgs[2].role, remo::contract::message::Role::User);
    assert_eq!(msgs[2].text(), "hello");

    // Suffix system
    let last = msgs.last().unwrap();
    assert_eq!(last.role, remo::contract::message::Role::System);
