...
use remo_runtime_contract::state::{StateKey, StateKeyOptions};
...
use remo_runtime::phase::ToolGateHook;
use remo_runtime::plugins::{Plugin, PluginDescriptor, PluginRegistrar};
use remo_runtime::registry::{AgentResolver, ResolvedAgent};
use remo_runtime::runtime::{AgentRuntime, RunActivation};
use remo_runtime::{PhaseContext, StateCommand};
...
async fn prior_tool_state_allows_later_guarded_tool_in_same_step() {
    ...
    assert_eq!(
        ...
        vec![
            (
                "u1",
                remo_runtime_contract::contract::suspension::ToolCallOutcome::Succeeded
            ),
            (
                "g1",
                remo_runtime_contract::contract::suspension::ToolCallOutcome::Succeeded
...
async fn guarded_tool_before_unlock_still_blocks_same_step() {
    ...
    assert_eq!(
        ...
        vec![
            (
                "g1",
                remo_runtime_contract::contract::suspension::ToolCallOutcome::Failed,
            ),
            (
                "u1",
                remo_runtime_contract::contract::suspension::ToolCallOutcome::Failed,
