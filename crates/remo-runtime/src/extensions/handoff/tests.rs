...
fn plugin_registers_state_key() {
    ...
    assert!(registry.keys_by_name.contains_key(
        <remo_runtime_contract::contract::active_agent::ActiveAgentIdKey as crate::state::StateKey>::KEY,
    ));
    assert_eq!(
        registry
            .keys_by_name
    ...
    assert_eq!(
        registry
            .keys_by_name
            .get(<remo_runtime_contract::contract::active_agent::ActiveAgentIdKey as crate::state::StateKey>::KEY)
            .map(|reg| reg.scope),
        Some(KeyScope::Thread)
    );
}
