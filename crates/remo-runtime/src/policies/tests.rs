...
use remo_runtime_contract::contract::lifecycle::{RunStatus, StopConditionSpec};
use remo_runtime_contract::model::Phase;
use remo_runtime_contract::{StateError, now_ms};
...
async fn stats_persist_across_store_restore() {
    ...
    store2
        .restore_persisted(persisted, remo_runtime_contract::UnknownKeyPolicy::Error)
        .unwrap();
