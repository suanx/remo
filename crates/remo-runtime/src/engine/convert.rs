//! Conversion between remo types and genai types.
...
use remo_runtime_contract::contract::tool_schema::sanitize_for_llm;

// ---------------------------------------------------------------------------
// Message → ChatMessage
// ---------------------------------------------------------------------------

/// Convert an remo `Message` to a genai `ChatMessage`.
...
/// Convert remo `ContentBlock`s to genai `ContentPart`s.
    ...
    blocks
        ...
        .filter_map(|b| match b {
            ...
            ContentBlock::Image { source } => match source {
                remo_runtime_contract::contract::content::ImageSource::Url { url } => {
                    Some(ContentPart::from_binary_url("image/png", url, None))
                }
                remo_runtime_contract::contract::content::ImageSource::Base64 {
                    media_type,
                    data,
            ...
            ContentBlock::Document { source, .. } => match source {
                remo_runtime_contract::contract::content::DocumentSource::Url { url } => {
                    Some(ContentPart::from_binary_url("application/pdf", url, None))
                }
                remo_runtime_contract::contract::content::DocumentSource::Base64 {
                    media_type,
                    data,
                } => Some(ContentPart::from_binary_base64(
                    media_type,
            ...
            ContentBlock::Audio { source } => match source {
                remo_runtime_contract::contract::content::AudioSource::Url { url } => {
                    Some(ContentPart::from_binary_url("audio/mpeg", url, None))
                }
                remo_runtime_contract::contract::content::AudioSource::Base64 {
                    media_type,
                    data,
                } => Some(ContentPart::from_binary_base64(
                    media_type,
            ...
            ContentBlock::Video { source } => match source {
                remo_runtime_contract::contract::content::VideoSource::Url { url } => {
                    Some(ContentPart::from_binary_url("video/mp4", url, None))
                }
                remo_runtime_contract::contract::content::VideoSource::Base64 {
                    media_type,
                    data,
                } => Some(ContentPart::from_binary_base64(
                    media_type,
...
/// Convert an remo `ToolDescriptor` to a genai `Tool`.
...
pub fn build_chat_request(
    ...
    if !system.is_empty() {
        let text = remo_runtime_contract::contract::content::extract_text(system);
        if !text.is_empty() {
...
// ---------------------------------------------------------------------------
// genai types → remo types
// ---------------------------------------------------------------------------

/// Map genai `StopReason` to remo `StopReason`.
...
/// Map genai `Usage` to remo `TokenUsage`.
...
/// Convert a genai `ToolCall` to an remo `ToolCall`.
...
mod tests {
    ...
    fn from_genai_tool_call_roundtrip() {
        ...
        let remo_call = from_genai_tool_call(&genai_call);
        assert_eq!(remo_call.id, "c1");
        assert_eq!(remo_call.name, "search");
        assert_eq!(remo_call.arguments, json!({"q": "rust"}));
    }
    ...
    fn from_genai_tool_call_preserves_arguments() {
        ...
        let remo_call = from_genai_tool_call(&genai_call);
        assert_eq!(remo_call.arguments, complex_args);
    }
