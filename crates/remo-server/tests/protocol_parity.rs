//! Protocol parity tests for the Remo server.
//!
...
fn make_app() -> axum::Router {
    ...
    let runtime = {
        let builder = AgentRuntimeBuilder::new()
            ...
            .with_commit_coordinator(
                coordinator
                    as Arc<
                        dyn remo_server_contract::contract::commit_coordinator::CommitCoordinator,
                    >,
            )
    ...
    let mailbox = std::sync::Arc::new(remo_server::mailbox::Mailbox::new(
        runtime.clone(),
        mailbox_store,
        Arc::clone(&store) as Arc<dyn remo_server_contract::contract::storage::ThreadRunStore>,
        "test".to_string(),
