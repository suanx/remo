...
//!
//! Exposes Remo agents as MCP tools so that external MCP clients
...
///
/// Each agent returned by [`remo_runtime::AgentResolver::agent_ids`] becomes one MCP tool
        ...
        let description = match resolver.resolve(agent_id) {
            Ok(agent) => {
                ...
                if prompt.len() > 200 {
                    ...
                    format!("Remo agent: {agent_id}")
                } else {
                    prompt.clone()
                }
            }
            Err(_) => format!("Remo agent: {agent_id}"),
        };
...
/// Create and start an MCP server backed by all known Remo agents.
///
...
mod tests {
    ...
    fn config_registers_all_agents_as_tools() {
        ...
        assert_eq!(config.name(), "remo-mcp");
