...
use remo_server_contract::contract::storage::{RunRecord, RunWaitingState, ThreadRunStore};
...
mod tests {
    ...
    fn convert_ignores_assistant_messages() {
        ...
        assert_eq!(
            converted[0].role,
            remo_server_contract::contract::message::Role::User
        );
    ...
    fn convert_preview_keeps_assistant_messages() {
        ...
        assert_eq!(
            converted[1].role,
            remo_server_contract::contract::message::Role::Assistant
        );
    ...
    fn is_resume_only_false_when_has_messages() {
        let req = ProcessedRequest {
            thread_id: "t1".into(),
            messages: vec![remo_server_contract::contract::message::Message::user(
                "hello",
            )],
            decisions: vec![(
                "tc1".into(),
