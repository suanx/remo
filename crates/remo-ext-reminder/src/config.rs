...
use remo_tool_pattern::{FieldCondition, MatchOp, PathSegment, parse_pattern};
...
fn parse_message_entry(
    ...
    let msg = match entry.target.as_str() {
        ...
        "session" => ContextMessage::session(
            &key,
            remo_runtime_contract::contract::message::Role::System,
            &entry.content,
        ),
...
mod tests {
    ...
    fn config_glob_tool_pattern() {
        ...
        assert!(matches!(
            ...
            remo_tool_pattern::ToolMatcher::Glob(_)
        ));
    }
