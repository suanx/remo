...
impl TypedTool for GenerateImageTool {
    ...
    fn description(&self) -> &str {
        "Generate an image from a text prompt. Supports OpenAI DALL-E 3, \
         Agnes AI Image, and OpenAI-compatible APIs."
    }
    ...
    async fn execute(
        ...
        let (base_url, api_key) = match provider {
            ...
            "agnes" => {
                let key = config.agnes_api_key.clone()
                    .or_else(|| args.api_key.clone())
                    .ok_or_else(|| ToolError::ExecutionFailed(
                        "Agnes AI API key not configured.".into()
                    ))?;
            ...
            _ => {
                let key = args.api_key.clone()
                    ...
                    .or_else(|| config.agnes_api_key.clone())
                    .ok_or_else(|| ToolError::ExecutionFailed(
                        "API key required. Set api_key in arguments or configure a provider.".into()
                    ))?;
                let url = args.base_url.clone()
                    .or_else(|| config.custom_base_url.clone())
        ...
        if !status.is_success() {
            ...
            return Err(ToolError::ExecutionFailed(format!(
                "Image generation API returned HTTP {status}: {err_msg}"
            )));
        }
        ...
        tracing::info!(
            target: "remo::media::image",
            provider = %provider, model = %model, count = images.len(),
            "Image generated"
        );
...
impl TypedTool for GenerateVideoTool {
    ...
    fn description(&self) -> &str {
        "Generate a video from a text prompt. Supports Agnes AI Video and \
         OpenAI-compatible video generation APIs."
    }
    fn category(&self) -> Option<&str> { Some("media") }
    ...
    async fn execute(
        ...
        let (base_url, api_key) = match provider {
            ...
            _ => {
                let url = args.base_url.clone().or_else(|| config.custom_base_url.clone())
                    .ok_or_else(|| ToolError::ExecutionFailed("Base URL required.".into()))?;
        ...
        if !status.is_success() {
            ...
            return Err(ToolError::ExecutionFailed(format!(
                "Video API returned HTTP {status}: {err_msg}"
            )));
        }
